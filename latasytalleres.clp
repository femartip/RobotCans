; Base de Hechos:
;R=Robot LL=Latas L=Lata N=nivel B=bateria
...
(deffacts bh    ;;base de hechos 
    ...
    (C 8 3) 
    (C 8 1) 
    (R 2 3 0 nivel 0 LL L 3 1 L 3 3 L 6 4))   ;;Hechos dinamicos 
...
(defrule recargar
    (R ?c ?f ?b nivel ?n LL $?ll)
    (T ?c ?f)
    (test (= ?b 0))
    (profundidad-maxima ?prof)  ;Profundidad maxima
    (test (< ?n ?prof)) ;;Comprobar que no se ha superado la profundidad maxima
=>
    (assert (R ?c ?f (+ ?b 5) nivel (+ ?n 1) LL $?ll))   ;;Si celda a de arriba libre y dentro del limite se mueve
    (bind ?*nod-gen* (+ ?*nod-gen* 1)))

(defrule subir     ;;subir 
    (R ?c ?f ?b nivel ?n LL $?ll)     ;;Estado del robot y latas 
    ...
    (test (< ?n ?prof)) ;;Comprobar que no se ha superado la profundidad maxima
=>
(assert (R ?c (+ ?f 1) ?b nivel (+ ?n 1) LL $?ll))   ;;Si celda a de arriba libre y dentro del limite se mueve
(bind ?*nod-gen* (+ ?*nod-gen* 1)))   
...
(defrule bajar     ;;bajar
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (- ?f 1) ?b nivel (+ ?n 1) LL $?ll))    ;;Si celda de abajo libre y dentro del limite se mueve 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule izq    ;;izquierda
    ...
    (test (< ?n ?prof))
=>
(assert (R (- ?c 1) ?f ?b nivel (+ ?n 1) LL $?ll))    ;;Si celda a izquierda libre y dentro del limite se mueve
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule der    ;;derecha 
    ...
    (test (< ?n ?prof))
=>
(assert (R (+ ?c 1) ?f ?b nivel (+ ?n 1) LL $?ll))    ;;Si celda a derecha libre y dentro de limites se mueve 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))

(defrule sublata     ;;subir lata 
    (R ?c ?f ?b nivel ?n LL $?r1 L ?cl ?fl $?r2)     ;;Estado del robot y latas 
    (D ?cf ?ff)  ;;Dimensiones estaticas del tablero 
    (profundidad-maxima ?prof) ;profundidad maxima
    (test (> ?b 0))
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (+ ?f 1) (- ?b 1) nivel (+ ?n 1) LL $?r1 L ?cl (+ ?fl 1) $?r2))   ;;Si celda a de arriba libre y dentro del limite, mueve tanto al robto como a al lata 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule bajlata     ;;bajar lata
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (- ?f 1) (- ?b 1) nivel (+ ?n 1) LL $?r1 L ?cl (- ?fl 1) $?r2))    ;;Si celda de abajo libre y dentro del limite, mueve tanto al robot como a la lata  
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule izqlata    ;;izquierda lata
    ...
    (test (< ?n ?prof))
=>
(assert (R (- ?c 1) ?f (- ?b 1) nivel (+ ?n 1) LL $?r1 L (- ?cl 1) ?fl $?r2))    ;;Si celda a izquierda libre y dentro del limite, mueve tanto al robot como a la lata 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule derlata    ;;derecha lata
    ...
    (test (< ?n ?prof))
=>
(assert (R (+ ?c 1) ?f (- ?b 1) nivel (+ ?n 1) LL $?r1 L (+ ?cl 1) ?fl $?r2))   ;;Si celda a derecha libre y dentro de limites, mueve tanto al robot como a la lata
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule sublatatriturar     ;;triturar lata arriba  
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (+ ?f 1) (- ?b 1) nivel (+ ?n 1) LL $?r1 $?r2))   ;;Si celda a de arriba libre y dentro del limite, mueve tanto al robto como a al lata 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule bajlatatriturar     ;;triturar lata abajo 
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (- ?f 1) (- ?b 1) nivel (+ ?n 1) LL $?r1 $?r2))    ;;Si celda de abajo libre y dentro del limite, mueve tanto al robot como a la lata  
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule izqlatatriturar    ;;izquierda lata
    ...
    (test (< ?n ?prof))
=>
(assert (R (- ?c 1) ?f (- ?b 1) nivel (+ ?n 1) LL $?r1 $?r2))    ;;Si celda a izquierda libre y dentro del limite, mueve tanto al robot como a la lata 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule derlatatriturar    ;;derecha lata
    ...
    (test (< ?n ?prof))
=>
(assert (R (+ ?c 1) ?f (- ?b 1) nivel (+ ?n 1) LL $?r1 $?r2))   ;;Si celda a derecha libre y dentro de limites, mueve tanto al robot como a la lata
(bind ?*nod-gen* (+ ?*nod-gen* 1)))


(defrule objetivo
    (declare (salience 100))
    ?t <- (R ?cr ?fr ?b nivel ?n LL) 
    =>
