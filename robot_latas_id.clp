...
(deffacts bh    ;;base de hechos 
    ...
    (C 8 3 1) 
    (C 8 1 1) 
    (R 2 3 2 nivel 0 LL L 3 1 L 3 3 L 6 4))   ;;Hechos dinamicos 
...
(defrule subir     ;;subir 
    (R ?c ?f ?id nivel ?n LL $?ll)     ;;Estado del robot y latas 
...
=>
(assert (R ?c (+ ?f 1) ?id nivel (+ ?n 1) LL $?ll))   ;;Si celda a de arriba libre y dentro del limite se mueve
...
(defrule bajar     ;;bajar
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (- ?f 1) ?id nivel (+ ?n 1) LL $?ll))    ;;Si celda de abajo libre y dentro del limite se mueve 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule izq    ;;izquierda
    ...
    (test (< ?n ?prof))
=>
(assert (R (- ?c 1) ?f ?id nivel (+ ?n 1) LL $?ll))    ;;Si celda a izquierda libre y dentro del limite se mueve
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule der    ;;derecha 
    ...
    (test (< ?n ?prof))
=>
(assert (R (+ ?c 1) ?f ?id nivel (+ ?n 1) LL $?ll))    ;;Si celda a derecha libre y dentro de limites se mueve 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))

(defrule sublata     ;;subir lata 
    (R ?c ?f ?id nivel ?n LL $?r1 L ?cl ?fl $?r2)     ;;Estado del robot y latas 
    (D ?cf ?ff)  ;;Dimensiones estaticas del tablero 
    (profundidad-maxima ?prof) ;profundidad maxima
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (+ ?f 1) ?id nivel (+ ?n 1) LL $?r1 L ?cl (+ ?fl 1) $?r2))   ;;Si celda a de arriba libre y dentro del limite, mueve tanto al robto como a al lata 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule bajlata     ;;bajar lata
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (- ?f 1) ?id nivel (+ ?n 1) LL $?r1 L ?cl (- ?fl 1) $?r2))    ;;Si celda de abajo libre y dentro del limite, mueve tanto al robot como a la lata  
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule izqlata    ;;izquierda lata
    ...
    (test (< ?n ?prof))
=>
(assert (R (- ?c 1) ?f ?id nivel (+ ?n 1) LL $?r1 L (- ?cl 1) ?fl $?r2))    ;;Si celda a izquierda libre y dentro del limite, mueve tanto al robot como a la lata 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule derlata    ;;derecha lata
    ...
    (test (< ?n ?prof))
=>
(assert (R (+ ?c 1) ?f ?id nivel (+ ?n 1) LL $?r1 L (+ ?cl 1) ?fl $?r2))   ;;Si celda a derecha libre y dentro de limites, mueve tanto al robot como a la lata
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule sublatatriturar     ;;triturar lata arriba  
    ...
    (R ?c ?f ?id nivel ?n LL $?r1 L ?cl ?fl $?r2)     ;;Estado del robot y latas 
    (profundidad-maxima ?prof) ;profundidad maxima   
    (C ?c =(+ ?f 2) ?d) ;;Comprueba que se puede mover ya que no hay ningun contenedor 
    (test (< ?d ?id))
    (test (and (= ?cl ?c) (= ?fl (+ ?f 1))))    ;;Comprueba si hay una lata arriba
    (test (< ?n ?prof))
=>
(assert (R ?c (+ ?f 1) ?id nivel (+ ?n 1) LL $?r1 $?r2))   ;;Si celda a de arriba libre y dentro del limite, mueve tanto al robto como a al lata 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule bajlatatriturar     ;;triturar lata abajo 
    ...
    (test (< ?n ?prof))
=>
(assert (R ?c (- ?f 1) ?id nivel (+ ?n 1) LL $?r1 $?r2))    ;;Si celda de abajo libre y dentro del limite, mueve tanto al robot como a la lata  
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule izqlatatriturar    ;;izquierda lata
    ...
    (R ?c ?f ?id nivel ?n LL $?r1 L ?cl ?fl $?r2)   ;;Estado del robot y latas  
    (profundidad-maxima ?prof) ;profundidad maxima
    (C = (- ?c 2) ?f ?d) ;;Comprueba que se puede mover ya que no hay ningun contenedor 
    (test (< ?d ?id))
    (test (and (= ?cl (- ?c 1)) (= ?fl ?f)))    ;;Comprueba si hay una lata a la izquierda 
    (test (< ?n ?prof))
=>
(assert (R (- ?c 1) ?f ?id nivel (+ ?n 1) LL $?r1 $?r2))    ;;Si celda a izquierda libre y dentro del limite, mueve tanto al robot como a la lata 
(bind ?*nod-gen* (+ ?*nod-gen* 1)))
...
(defrule derlatatriturar    ;;derecha lata
    ...
    (test (< ?n ?prof))
=>
(assert (R (+ ?c 1) ?f ?id nivel (+ ?n 1) LL $?r1 $?r2))   ;;Si celda a derecha libre y dentro de limites, mueve tanto al robot como a la lata
(bind ?*nod-gen* (+ ?*nod-gen* 1)))


(defrule objetivo
    (declare (salience 100))
    ?t <- (R ?cr ?fr ?id nivel ?n LL) 
    =>
